...
export default function BlogPage() {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      const response = await fetch(
        `https://mamun-reza-freeshops-backend.vercel.app/api/v1/admin/allBlogPage?page=${page}&limit=${limit}`
      );
      if (!response.ok) {
        throw new Error('Failed to fetch blogs');
      }
      const responseData = await response.json();
      const blogsData = Array.isArray(responseData?.data)
        ? responseData.data
        : responseData?.data
        ? [responseData.data]
        : [];
  ...
  const handleEdit = async (id) => {
    try {
      ...
      setNewBlog({
        ...
        newsImage: data?.data?.newsImage ? new Blob([data.data.newsImage]) : null
      });
      setEditBlogId(id);
      setIsEditMode(true);
      setIsModalOpen(true);
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('newsTitle', newBlog.newsTitle || '');
    formData.append('newsDescription', newBlog.newsDescription || '');
    formData.append('heading', newBlog.heading || '');
    if (newBlog.image) {
      formData.append('image', newBlog.image);
    }
    if (newBlog.newsImage) {
      formData.append('newsImage', newBlog.newsImage);
    }
    ...
    try {
      ...
      setIsEditMode(false);
      setEditBlogId(null);
      setNewBlog({ title: '', description: '', image: null, imageUrl: '', newsTitle: '', newsDescription: '', heading: '', newsImage: null });
    } catch (err) {
      alert(err.message);
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      <div className="rounded-t-xl p-4 flex justify-between items-center mb-4" >
        <h1 className="text-2xl font-semibold text-black">Blog Page</h1>
          ...
          <div className="bg-white rounded-lg w-[500px] p-6 relative shadow-2xl max-h-[80vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={newBlog.newsTitle}
                  onChange={(e) => setNewBlog(prev => ({ ...prev, newsTitle: e.target.value }))}
              ...
              <div>
                ...
                <textarea
                  value={newBlog.newsDescription}
                  onChange={(e) => setNewBlog(prev => ({ ...prev, newsDescription: e.target.value }))}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewBlog(prev => ({ ...prev, heading: e.target.value }))}
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="p-4 border-t flex items-center justify-between text-sm text-gray-600">
          <div>
            Showing {blogs.length > 0 ? (page - 1) * limit + 1 : 0}-{Math.min(page * limit, totalCount)} of {totalCount}
          </div>
